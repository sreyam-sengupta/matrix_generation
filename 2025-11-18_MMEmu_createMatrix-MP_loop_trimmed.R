...
base_output_dir <- "/p/projects/magpie/users/sreyamse/magpie/projects/PIK_2025-07-02/magpie/output/Matrix_test_rev1/SSP2_BD00"

# Scenario name prefix
scenario_name <- "SSP2_BD00"

# Run folder pattern: SSP2_BD00_BE{BE_price}_G{GHG_price}demand_rev1
run_suffix_demand <- "demand_rev1"  # suffix for demand-driven runs
run_suffix_price <- "G0000price_rev1"  # suffix for price-driven runs (for bioenergy prices)

# BE price values (used in folder names: SSP2_BD00_BE{xx}_G{yyyy}demand_rev1)
...
# Output folder for matrix files
matrix_output_dir <- "/p/projects/magpie/users/sreyamse/running_output/matrix_output"

# Full mapping file for MESSAGE (we'll filter output after mapping)
map_file <- "/p/projects/magpie/users/sreyamse/running_output/data/2025-11-28_map_magpie_message_human_corrected.csv"

# Variables to keep (MESSAGE variable names after accounting for name changes)
vars_to_keep_file <- "/p/projects/magpie/users/sreyamse/running_output/data/2025-11-18_vars_to_keep.csv"
...
# Running all phases from scratch to fix raw phase issues
...
matrix_file <- "/p/projects/magpie/users/sreyamse/running_output/matrix_output/2025-11-28_magpie_input_SSP2_BD00_human_corrected_v2.csv"
...
for (be_idx in seq_along(be_price_values)) {
  ...
  for (ghg_idx in seq_along(ghg_price_values)) {
    ...
    # Raw files are in SSP2_BD00 subfolder
    of_raw <- file.path(matrix_output_dir, "SSP2_BD00", paste0(scenario_name, "_BE", be_str, "_GHG", str_pad(ghg, 3, pad = "0"), "raw.csv"))
    of_map <- file.path(matrix_output_dir, "SSP2_BD00", paste0(scenario_name, "_BE", be_str, "_GHG", str_pad(ghg, 3, pad = "0"), "map.csv"))
